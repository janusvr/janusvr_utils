...
FString GetTextureFormatExtension(JanusTextureFormat format)
{
	switch (format)
	{
...
void DecodeAndSaveHQLightmap(LightmappedObjects* Exported, FString RootFolder, JanusTextureFormat Format)
{
...
{
	JanusExporterOptions options;
	options.MaterialResolution = 1024;
	options.LightmapFormat = JanusTextureFormat::OpenEXR;
		...
		for (int32 i = 0; i < StaticMeshes.Num(); i++)
		{
			...
			if (LightMap != NULL)
			{
				...
				if (!TexturesExp.Contains(TexName))
				{
					...
					AssetImage Asset;
					Asset.Format = options.LightmapFormat;
					Asset.Path = TexName;
			...
			for (int32 j = 0; j < Materials.Num(); j++)
			{
				...
				FString MatExported = ExportMaterial(options.MaterialResolution, Root, Material, JanusTextureFormat::PNG);
	...
	{
		DecodeAndSaveHQLightmap(&Elem.Value, Root, options.LightmapFormat);
	}
	...
	for (int32 i = 0; i < AssetImages.Num(); i++)
	{
		...
		Index.Append("\n\t\t\t\t<AssetImage id=\"" + Asset.Path + "\" src=\"" + Asset.Path + GetTextureFormatExtension(Asset.Format) + "\" />");
	}
